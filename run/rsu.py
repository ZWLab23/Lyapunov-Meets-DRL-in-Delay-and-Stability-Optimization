...
class PlotConfig:
    ...
    def __init__(self, number, weight) -> None:
        self.device = device
        self.config = VehicularEnvConfig()
        self.info = "Processing capacity of RSU"
        self.traffic = self.config.vehicle_number_dict.get(number)  # 读取车流信息
        self.weight = self.config.w.get(weight)  # 读取权重
        self.change = list(self.config.rsu_compute_speed.values())  # 读取RSU计算速度

        # 创建路径
        self.main_path = parent_path + "/results/" + "/rsu/" + "/weight={},speed={}/" \
            .format(self.traffic, self.weight) + current_time
...
def traffic_comparison():
    """ 不同RSU计算速度下的reward对比 """
    cfg_1 = SACConfig(number_tag="number_1", weight_tag="weight_2", speed_tag="speed_1", stability_tag="a",
                      flow_tag="nf")
    cfg_2 = SACConfig(number_tag="number_1", weight_tag="weight_2", speed_tag="speed_2", stability_tag="a",
                      flow_tag="nf")
    cfg_3 = SACConfig(number_tag="number_1", weight_tag="weight_2", speed_tag="speed_3", stability_tag="a",
                      flow_tag="nf")
    cfg_4 = SACConfig(number_tag="number_1", weight_tag="weight_2", speed_tag="speed_4", stability_tag="a",
                      flow_tag="nf")
    plot_cfg = PlotConfig(number="number_1", weight="weight_2")  # 画图参数
    ...
    # 创建保存结果和模型路径的文件夹
    make_dir(plot_cfg.model_path + '/speed_1/', plot_cfg.model_path + '/speed_2/',
             plot_cfg.model_path + '/speed_3/', plot_cfg.model_path + '/speed_4/')
    ...
    # 保存模型
    sac_mind_1.agent.save(plot_cfg.model_path + '/speed_1/')
    sac_mind_2.agent.save(plot_cfg.model_path + '/speed_2/')
    sac_mind_3.agent.save(plot_cfg.model_path + '/speed_3/')
    sac_mind_4.agent.save(plot_cfg.model_path + '/speed_4/')
    ...
    rewards_1, ma_rewards_1, growths_1, t_taus_1, _ = sac_mind_1.test()
    rewards_2, ma_rewards_2, growths_2, t_taus_2, _ = sac_mind_2.test()
    rewards_3, ma_rewards_3, growths_3, t_taus_3, _ = sac_mind_3.test()
    rewards_4, ma_rewards_4, growths_4, t_taus_4, _ = sac_mind_4.test()
